...
class SqlServerLiquidityDerivativesStore:
    ...
    def __init__(
        ...
        *,
        actor: str = "ThesisPulse.AI.LiquidityDerivatives",
    ...
    def get_latest(
        ...
        try:
            ...
            return StoredLiquidityDerivativesOutput(
                engine_output_id=output_id,
                output=LiquidityDerivativesContextOutputV1.model_validate_json(
                    row[1]
                ),
    ...
    def _claim_inbox(
        ...
        causation_id = (
            UUID(metadata.causation_id) if metadata.causation_id else None
        )
        ...
        if int(row[1]) >= int(row[2]):
            raise RuntimeError("Liquidity Context inbox retry limit was reached")
        cursor.execute(
            """
    ...
    def _load_candle_window(
        ...
        sql = f"""
            ...
            (
                SELECT candle.[candle_id], candle.[open_at_utc],
                    ...
                    candle.[open_interest], candle.[revision],
                    candle.[received_at_utc], candle.[quality_status],
                    candle.[is_usable_for_new_exposure],
                    ROW_NUMBER() OVER
                    (
            ...
            SELECT TOP ({self._maximum_input_count}) [candle_id], [open_at_utc],
                ...
                [close_price], [volume_qty], [open_interest], [revision],
        ...
        result = [
            LiquidityDerivativesCandle(
                ...
                revision=int(row[9]),
                received_at_utc=_as_utc(row[10]),
                quality_status=str(row[11]),
                is_usable_for_new_exposure=bool(row[12]),
            )
            for row in rows
        ]
        result.reverse()
    ...
    def _resolve_engine_id(self, cursor: pyodbc.Cursor) -> int:
        ...
        if row is None:
            raise RuntimeError(
                f"Active Liquidity Context engine '{self._engine_code}' was not found"
            )
        return int(row[0])
    ...
    def _read_by_source_message(
        ...
        return StoredLiquidityDerivativesOutput(
            engine_output_id=output_id,
            output=LiquidityDerivativesContextOutputV1.model_validate_json(row[1]),
            input_candle_ids=tuple(int(item[0]) for item in inputs),
        )
    ...
    def _insert_engine_output(
        ...
        metadata_json = json.dumps(
            {
                ...
                "liquidityPoolCount": len(output.liquidity_pools),
                "derivativesState": output.derivatives_state,
        ...
        missing.extend(["optionChain", "futuresBasis"])
        return int(
            cursor.execute(
                """
