...
Run("complete authoritative lifecycle passes", () =>
{
    ...
    var report = evaluator.Evaluate(
        ...
        detail,
        CompleteLineage(detail, now),
        SafeOperationalStates(now),
        now);
...
Run("missing P&L evidence remains incomplete", () =>
{
    ...
    var lineage = CompleteLineage(incomplete, now)
...
Run("self-referential causation fails", () =>
{
    ...
    var thesisIndex = lineage.FindIndex(item => item.Stage == "THESIS");
    lineage[thesisIndex] = lineage[thesisIndex] with
    {
        CausationUid = lineage[thesisIndex].EntityUid,
    };
...
Run("restrictive mode cannot allow new exposure", () =>
{
    ...
    {
        new PaperTradeLifecycleOperationalStateV1(
            "STRATEGY",
            detail.Summary.StrategyCode,
            "CLOSE_ONLY",
            Guid.NewGuid(),
            true,
            true,
...
{
    var detail = CompleteDetail(now);
    var stale = detail with
    {
        Summary = detail.Summary with { IsStale = true },
    };

    var report = evaluator.Evaluate(
        PaperTradeLifecycleContractV1.PaperEnvironment,
        stale,
        CompleteLineage(stale, now),
        SafeOperationalStates(now),
        now);
...
await RunAsync("non-SQL acceptance store fails closed", async () =>
{
    IPaperTradeLifecycleAcceptanceStore store =
        new UnavailablePaperTradeLifecycleAcceptanceStore();
...
static PaperTradeLifecycleDetailV1 CompleteDetail(DateTimeOffset now)
{
    var summary = new PaperTradeLifecycleSummaryV1(
        ...
        "LONG",
        "PNL_VALUED",
        PaperTradeLifecycleContractV1.Complete,
        true,
        false,
        now.AddSeconds(-1),
        now,
    ...
    DateTimeOffset now)
{
    ...
    return
    [
        Evidence("SIGNAL", summary.SignalUid, summary.CorrelationUid, null, now.AddMinutes(-9),
            "intelligence.signals"),
        Evidence("THESIS", summary.ThesisUid!.Value, summary.CorrelationUid, Guid.NewGuid(),
            now.AddMinutes(-8), "thesis.theses"),
        Evidence("RISK", summary.RiskDecisionUid!.Value, summary.CorrelationUid, Guid.NewGuid(),
            now.AddMinutes(-7), "risk.risk_decisions"),
        Evidence("TRADE_PLAN", summary.TradePlanUid, summary.CorrelationUid, Guid.NewGuid(),
            now.AddMinutes(-6), "risk.trade_plans"),
        Evidence("EXECUTION_COMMAND", summary.ExecutionCommandUid!.Value,
            summary.CorrelationUid, Guid.NewGuid(), now.AddMinutes(-5),
            "execution.execution_commands"),
        Evidence("ORDER_EVENT", Guid.NewGuid(), summary.CorrelationUid, Guid.NewGuid(),
            now.AddMinutes(-4), "execution.order_events"),
        Evidence("FILL", Guid.NewGuid(), summary.CorrelationUid, Guid.NewGuid(),
            now.AddMinutes(-3), "execution.fills"),
        Evidence("POSITION_EVENT", Guid.NewGuid(), summary.CorrelationUid, Guid.NewGuid(),
            now.AddMinutes(-2), "portfolio.position_events"),
        Evidence("PNL", summary.PnlSnapshotUid!.Value, summary.CorrelationUid, null,
            now.AddMinutes(-1), "portfolio.pnl_snapshots"),
    ];
}
...
static IReadOnlyCollection<PaperTradeLifecycleOperationalStateV1> SafeOperationalStates(
    DateTimeOffset now) =>
[
    new(
        "PLATFORM",
