...
class SqlServerMarketRegimeStore:
    ...
    def __init__(
        ...
        *,
        actor: str = "ThesisPulse.AI.Regime",
    ...
    def process(
        ...
        if source.engine_output_id is None:
            raise ValueError("SQL regime processing requires a persisted feature output")
        ...
        try:
            ...
            duplicate = self._read_by_source(
                cursor,
                engine_id,
    ...
    def _resolve_source(self, cursor: pyodbc.Cursor, output_id: int) -> _SourceOutput:
        row = cursor.execute(
            ...
            SELECT output.[instrument_id], output.[message_uid], output.[correlation_id],
                ...
                output.[data_quality_status], output.[is_stale]
    ...
    def _validate_source(
        ...
        if not snapshot.is_eligible_for_engines:
            raise ValueError("Feature output is not eligible for regime processing")
        ...
        if source_row.expires_at_utc <= processed_at_utc:
            raise ValueError("Feature output expired before regime processing")
    ...
    def _resolve_engine_id(self, cursor: pyodbc.Cursor) -> int:
        ...
        if row is None:
            raise RuntimeError(
                f"Active market regime engine '{self._engine_code}' was not found"
            )
        return int(row[0])
    ...
    def _insert_engine_output(
        ...
        source: StoredFeatureSnapshot,
        output: MarketRegimeOutputV1,
        existing: ExistingRegimeRevision | None,
    ) -> int:
        ...
        metadata_json = json.dumps(
            {
                ...
                "structureRegime": output.structure_regime,
                "volatilityRegime": output.volatility_regime,
    ...
    @staticmethod
    def _stored_from_row(row: pyodbc.Row) -> StoredRegimeOutput:
        return StoredRegimeOutput(
            engine_output_id=int(row[0]),
            output=MarketRegimeOutputV1.model_validate_json(row[1]),
