    ...
    SignalRiskPersistenceOptions persistenceOptions) : IAutomaticTradePlanResultStore
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            var lineage = await ResolveLineageAsync(connection, transaction, command, cancellationToken);
            ...
            if (existing is not null)
            {
                ...
                return new AutomaticTradePlanPersistenceResult("DUPLICATE", existing.Value.TradePlanUid, existing.Value.TradePlanId);
            }

            var plan = result.TradePlan;
            var rawJson = JsonSerializer.Serialize(plan, JsonOptions);
            var contractHash = Convert.ToHexString(SHA256.HashData(Encoding.UTF8.GetBytes(rawJson))).ToLowerInvariant();
            ...
            var tradePlanId = await InsertPlanAsync(
                ...
                contractHash,
                cancellationToken);
            await InsertTargetsAsync(connection, transaction, tradePlanId, plan.Targets, cancellationToken);
            ...
            await transaction.CommitAsync(cancellationToken);
            return new AutomaticTradePlanPersistenceResult("CREATED", plan.TradePlanUid, tradePlanId);
        }
        catch
        {
    ...
    {
        const string sql = """
            SELECT e.[signal_risk_evaluation_id], e.[signal_id], s.[instrument_id], t.[thesis_id]
            FROM [risk].[signal_risk_evaluations] e WITH (UPDLOCK, HOLDLOCK)
            INNER JOIN [intelligence].[signals] s ON s.[signal_id] = e.[signal_id]
            INNER JOIN [thesis].[theses] t
                ON t.[thesis_uid] = @thesis_uid
        ...
        sqlCommand.Parameters.Add("@risk_decision_uid", SqlDbType.UniqueIdentifier).Value = command.RiskDecisionUid;
        sqlCommand.Parameters.Add("@signal_uid", SqlDbType.UniqueIdentifier).Value = command.SignalUid;
        sqlCommand.Parameters.Add("@thesis_uid", SqlDbType.UniqueIdentifier).Value = command.ThesisUid;
        await using var reader = await sqlCommand.ExecuteReaderAsync(cancellationToken);
        if (!await reader.ReadAsync(cancellationToken))
            throw new InvalidOperationException("Approved authoritative Risk, Signal and Thesis lineage was not found.");
        ...
        CancellationToken cancellationToken)
    {
        ...
        const string sql = """
            ...
            (
                ...
                [thesis_id], [signal_id], [instrument_id], [contract_version], [environment],
                [source_service], [source_version], [plan_version], [side], [position_intent],
                [entry_order_type], [entry_reference_price], [entry_limit_price], [entry_trigger_price],
                ...
                [allow_break_even_move], [allow_time_exit], [allow_signal_exit], [exit_policy_version],
                [execution_policy_version], [initial_status], [status_reasons_json], [generated_at_utc],
                [valid_until_utc], [supersedes_trade_plan_uid], [is_current], [correlation_id],
                [causation_id], [metadata_json], [raw_contract_json], [contract_hash], [created_by]
            )
            ...
            (
                ...
                @thesis_id, @signal_id, @instrument_id, @contract_version, @environment,
                'ThesisPulse.Risk.Service', @source_version, 1, @side, @position_intent,
                @entry_order_type, @entry_reference_price, @entry_limit_price, @entry_trigger_price,
                @minimum_acceptable_price, @maximum_acceptable_price, @approved_quantity,
                @minimum_execution_quantity, @allow_partial_fill, @stop_loss_price,
                ...
                @new_entry_cutoff_utc, @mandatory_exit_by_utc, @allow_trailing_stop,
                @allow_break_even_move, @allow_time_exit, @allow_signal_exit, @exit_policy_version,
                @execution_policy_version, 'READY', N'[]', @generated_at_utc,
                @valid_until_utc, NULL, 1, @correlation_id,
                @causation_id, @metadata_json, @raw_contract_json, @contract_hash, N'ThesisPulse.Risk.Service'
            );
            """;
        await using var command = new SqlCommand(sql, connection, transaction);
        command.Parameters.Add("@trade_plan_uid", SqlDbType.UniqueIdentifier).Value = plan.TradePlanUid;
        command.Parameters.Add("@message_uid", SqlDbType.UniqueIdentifier).Value = source.SourceMessageUid;
        command.Parameters.Add("@evaluation_id", SqlDbType.BigInt).Value = lineage.SignalRiskEvaluationId;
        command.Parameters.Add("@thesis_id", SqlDbType.BigInt).Value = lineage.ThesisId;
        ...
        command.Parameters.Add("@contract_version", SqlDbType.VarChar, 20).Value = TradePlanContractV1.ContractVersion;
        command.Parameters.Add("@environment", SqlDbType.VarChar, 20).Value = plan.Environment;
        command.Parameters.Add("@source_version", SqlDbType.VarChar, 50).Value = result.BuilderVersion;
        command.Parameters.Add("@side", SqlDbType.VarChar, 10).Value = plan.Side;
        command.Parameters.Add("@position_intent", SqlDbType.VarChar, 20).Value = plan.PositionIntent;
        command.Parameters.Add("@entry_order_type", SqlDbType.VarChar, 20).Value = plan.Entry.OrderType;
        ...
        AddNullableDecimal(command, "@minimum_execution_quantity", plan.MinimumExecutionQuantity, 19, 6);
        ...
        command.Parameters.Add("@stop_loss_order_type", SqlDbType.VarChar, 20).Value = plan.StopLoss.OrderType;
        ...
        command.Parameters.Add("@trade_date", SqlDbType.Date).Value = plan.Session.TradeDate.ToDateTime(TimeOnly.MinValue);
        command.Parameters.Add("@not_before_utc", SqlDbType.DateTime2).Value =
            plan.Session.NotBeforeUtc?.UtcDateTime ?? (object)DBNull.Value;
        command.Parameters.Add("@new_entry_cutoff_utc", SqlDbType.DateTime2).Value = plan.Session.NewEntryCutoffUtc.UtcDateTime;
        command.Parameters.Add("@mandatory_exit_by_utc", SqlDbType.DateTime2).Value = plan.Session.MandatoryExitByUtc.UtcDateTime;
        command.Parameters.Add("@allow_trailing_stop", SqlDbType.Bit).Value = plan.ExitPolicy.AllowTrailingStop;
        command.Parameters.Add("@allow_break_even_move", SqlDbType.Bit).Value = plan.ExitPolicy.AllowBreakEvenMove;
        command.Parameters.Add("@allow_time_exit", SqlDbType.Bit).Value = plan.ExitPolicy.AllowTimeExit;
        command.Parameters.Add("@allow_signal_exit", SqlDbType.Bit).Value = plan.ExitPolicy.AllowSignalExit;
        command.Parameters.Add("@exit_policy_version", SqlDbType.VarChar, 50).Value = plan.ExitPolicy.PolicyVersion;
        command.Parameters.Add("@execution_policy_version", SqlDbType.VarChar, 50).Value = plan.ExecutionPolicyVersion;
        command.Parameters.Add("@generated_at_utc", SqlDbType.DateTime2).Value = plan.GeneratedAtUtc.UtcDateTime;
        command.Parameters.Add("@valid_until_utc", SqlDbType.DateTime2).Value = plan.ValidUntilUtc.UtcDateTime;
