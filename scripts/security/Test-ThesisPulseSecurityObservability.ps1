...
Assert-Contains $auth "jwt.SaveToken" "Operator authentication must keep raw session saving disabled."
Assert-Contains $auth "jwt.IncludeErrorDetails" "Authentication errors must keep detailed validation data hidden."
Assert-Contains $auth "ValidateLifetime" "Operator session validation must enforce lifetime."
Assert-Contains $auth "RequireExpirationTime" "Operator sessions must require expiration."
Assert-Contains $auth "RequireSignedTokens" "Operator sessions must require signed material."
